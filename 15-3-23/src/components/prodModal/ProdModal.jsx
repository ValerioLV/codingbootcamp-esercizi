...
const ProdModal = ({
	...
	const onHandleAddCart = () => {
		// if(setCartList.find((prev) => prev.includes(prodData.id))){
		...
		localStorage.setItem(
			"cartItems",
			JSON.stringify([...CartItems, prodData]),
		);
	...
	return (
		<div className="ProdModal">
			<div className="ProdModal__content">
				<button
					className="ProdModal__close"
					onClick={onHandleClose}>
					X
				</button>
				<div className="ProdModal__data">
					<h2 className="ProdModal__data--title">
						{prodData.title}
					</h2>
					<div className="ProdModal__images">
						{prodData.images.map((img) => (
							<img
								src={img}
								alt={prodData.title}
								key={img}
