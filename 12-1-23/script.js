...
// const userIdEl = todos.map((UID) => UID.userId);
// console.log(userIdEl);

// const filtredId = todos.filter((todoID) =>  
//    todoID.id >= 4).map((todoID) => todoID.id);
...
while(i < 10){
    ...
    const faretxt = document.createElement('p');
    const id = document.createElement('p');
    const list = document.body;

    fareEl.className = 'snlgContainer';
    faretxt.textContent = "Cosa da fare numero " + [i];
    id.textContent ="id: " + [i];

    fareEl.append(faretxt, id);
    list.appendChild(fareEl);
